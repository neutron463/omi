...
- Desktop Windows release pipeline: merging `desktop/windows/**` to `main` runs `.github/workflows/desktop_windows_release.yml` — patch-bumps `desktop/windows/package.json`, tags `v<version>-windows`, builds the NSIS installer on a windows runner (signed via Azure Trusted Signing when the `AZURE_*` secrets exist, else unsigned), and publishes the `.exe` + `.exe.blockmap` + `latest.yml` to a prerelease GitHub Release. Setup + secret names: `desktop/windows/docs/release-pipeline.md`.
