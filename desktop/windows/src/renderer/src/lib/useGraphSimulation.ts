...
export class GraphSimulation {
  ...
  // SPIKE (spike/win-memory-graph-3d): `dimensions` selects a 2D (default) or 3D
  // layout. 2D pins every node to z=0 so the camera faces it head on — what makes
  // labels reliably readable, since two nodes far apart in 3D can still project on
  // top of each other. Onboarding + the inline Memories card pass 2 (fixed
  // camera). Only the full-screen INTERACTIVE brain map passes 3, where
  // OrbitControls lets the user rotate to disambiguate depth.
  constructor(
