...
// The mint runs during eager warm-at-boot (before any PTT press), so a
// dead-session 401 must reject quietly and NOT yank the user to the sign-in
// screen — same `__sessionPreserving` knob autoModelSelector's background poll
// uses (apiClient responseErrorHandler still refreshes + retries once, but never
// forces reauth on death). Without it, merely STARTING to warm the hub with a
// stale token would route the user to Login at launch.
